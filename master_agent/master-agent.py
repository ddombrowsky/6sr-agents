...
def _build_sdex_revision_system_prompt():
    ...
    'file.\n\n'
    'PATCHING main.py: BEWARE DUPLICATE LINES. Some parent strategies inherited from '
    'older templates and still have a `decide_asset()` function with threshold lines '
    'identical to `decide()`\'s (e.g. `if buy_below and price <= buy_below:` appears '
    'in BOTH functions). When apply_patch tries to change one, it matches in two places '
    'and aborts the ENTIRE patch -- wasting a tool call. To avoid this: include the '
    'function signature line (`def decide(`) or enough unique surrounding context in '
    'the hunk so the patch targets only the function you mean. Or use `write_file` '
    'with the complete file. Found in the wild 2026-08-24: one revision wasted 5+ '
    'tool calls on repeated ambiguity errors before working around it.\n\n'
    'Do NOT write or modify `state.json`. It is maintained by main.py\'s tick loop '
    'and monitor.py\'s scoring system. Writing it manually (e.g. setting '
    '`balance_usd` to 1250) inflates the starting balance, corrupts the scoring '
    'comparison against the parent, and monitor.py deletes it before starting the '
    'strategy anyway. A freshly cloned strategy always starts from $1000.\n\n'
...
def _build_maker_revision_system_prompt():
    ...
    'runs at a larger size, so judge yourself on edge per fill, not on gross dollars '
    'traded.\n\n'
    ...
    f'SCORING, AND IT IS NOT NET WORTH. Fitness is {_FACTS["score_formula"]}.\n'
    f'Read from {_FACTS["score_source"]} -- NOT from state.json, so writing numbers into '
    f'your own state changes your score by exactly zero.\n'
    'Execution edge is measured per fill against the mid AT THE MOMENT THAT FILL '
    'HAPPENED: buying below the contemporaneous mid and selling above it is the only '
    'thing that scores. Holding XLM while the price rises earns you NOTHING -- that '
    'used to be most of the leaderboard and it is exactly what this scoring removed. '
    'Inventory you still hold is marked at the LOWER of cost or market, so an unsold '
    'loser costs you the moment the mid drops below what you paid; refusing to sell it '
    'does not hide the loss. Quoting so wide you never fill parks you at exactly '
    f'{_FACTS["starting_score"]:.0f}.\n'
    'The practical consequence: your enemy is adverse selection, not spread. A fill '
    'that happens because the mid was already moving through your quote scores '
    'NEGATIVE even though you "captured the spread". Widening, quoting less in fast '
    'markets, or skewing away from the side that keeps getting picked off are all real '
    'answers. Trading more is not automatically better.\n\n'
...
def _build_null_revision_system_prompt():
    ...
    'The template already maintains `state["recent"]` (a rolling window of '
    '[timestamp, delta] pairs updated by bank() after each answer), so reading '
    'it from decide() via `state.get(\'recent\', [])` is safe and any adaptive '
    'logic based on it will actually fire. However, if your decide() reads OTHER '
    'state keys the template does not populate (e.g. a rolling win rate, a '
    'per-tick counter), you must also add the code in main()\'s answer-handling '
    'section to maintain them -- reading a key that is never written makes your '
    'adaptive logic dead code, and the 120s smoke test cannot catch this because '
    'it runs too briefly for adaptation to matter.\n\n'
    ...
    'Your working directory is NOT the strategy directory, so either `cd` into the '
    "strategy path first (e.g. `cd /opt/strategies/clone_xxx && git checkout -b "
    'auto/<timestamp> && git add -A && git commit -m ...`) or use `git -C '
    '/opt/strategies/clone_xxx ...` for each command. The strategy directory already '
    'has a .git from the clone step, so `git init` is unnecessary. Running `git` '
    'without one of these fails with "not a git repository" and wastes a tool call.\n\n'
    'To change a few lines of a file, `apply_patch` is usually better than '
    '`write_file`: it takes a V4A patch (`*** Begin Patch` / `*** Update File: <path>` '
    '/ `@@` / `-` and `+` lines / `*** End Patch`) and edits in place. Use the FULL '
    'ABSOLUTE path in `*** Update File:` (e.g. `/opt/strategies/<name>/main.py`), not '
    'a relative path like `main.py` -- your working directory is not the strategy '
    'directory, so a relative path resolves against the wrong directory and the patch '
    'fails. Its `-` and context lines must match the file exactly as it is on disk '
    'right now -- a hunk that matches nothing aborts the whole patch and writes '
    'NOTHING, so `read_file` the target first and copy the lines from it. If a patch '
    'will not match after one retry, fall back to `write_file` with the complete '
    'file.\n\n'
    'The `calculate` tool only evaluates literal numeric expressions with +, -, *, /, '
    '** -- it cannot handle variables (e.g. `calculate(expression="(2-2*c)*(c-0.25)")` '
    'fails). Use `exec` with Python for symbolic math, or substitute numeric values '
    'and use `calculate` with those.\n\n'
...
def _build_yield_revision_system_prompt():
    ...
    f'CAPITAL DILUTION IS REAL. The replay engine prices every venue at a DILUTED '
    f'rate: rate * S/(S+D), where S is the pool\'s existing supply and D is the book '
    f'(${float(_FACTS["book_usd"]):.0f}). A deep pool (S >> D) earns nearly its quoted '
    f'rate; a shallow pool (S < D) earns a fraction of it. A venue quoting 12% over '
    f'~$12 of supplied capital earns only ~2% once your ${float(_FACTS["book_usd"]):.0f} '
    f'arrives, and the replay refuses to credit legs whose free_liquidity_usd is below '
    f'${float(_FACTS["min_free_liquidity_usd"]):.0f} at all. Rank by the rate your capital '
    f'actually earns after dilution, not the headline rate the pool quotes -- '
    f'min_free_liquidity_usd should be at least '
...
def _config_only_correction(strategy_path: Path) -> str:
    ...
    if _DOMAIN.NAME == 'yield':
        return (
            ...
            f'"main.py:{_entry_point()}" and that `excess_bp` is non-negative (zero matches '
            f'the null baseline), and write the '
...
def _explore_prompt_yield(strategy_name, strategy_path, leaderboard, venue_line) -> str:
    ...
    return (
        ...
        f'fallback -- and `excess_bp` should be non-negative (zero matches '
        f'the null baseline; positive means beating it). Run '
